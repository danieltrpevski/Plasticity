...
NEURON {
	POINT_PROCESS adaptive2_hetero_amp_inhexp2syn
	RANGE tau1, tau2, e, i, tau
	NONSPECIFIC_CURRENT i
	RANGE g, gmax
	RANGE delta_t, tlast, delta_theta
	RANGE weight, calcium_max, calcium_amp, learning_rate, n, n1, w0, active_syn_flag, last_wuf
	RANGE learning_rate_theta, theta_min, theta, sf
...
BREAKPOINT {
	...
	if (weight_update_flag == 0) {
	  ...
	  LOCAL s1, s2, s3, kernel_active, kernel_inactive
	  s1 = sigmoidal(sf*calcium_max, theta_min, n1)
	  
          if (active_syn_flag == 1) {	
          	s2 = sigmoidal(sf*calcium_max, theta, n)
	  	kernel_active = -1 * s1 * (2*s2 - 1)
	        weight = weight + learning_rate * kernel_active * delta_t
	  
	  } else {
	  	s3 = sigmoidal(sf*calcium_max, theta, n)
	  	kernel_inactive = s1 * (2*s3 - 1)
	  	weight = weight + learning_rate * kernel_inactive * delta_t
	  }
	  	
	  delta_theta = learning_rate_theta * calcium_max/calcium_amp * (sf*calcium_max - theta) * delta_t
	  theta = theta + delta_theta
	  :theta_min = theta_min + delta_theta
