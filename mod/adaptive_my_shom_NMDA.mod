...
NEURON {
	...
	RANGE learning_rate_thresh_LTP, learning_rate_thresh_LTPl, learning_rate_thresh_LTD, learning_rate_thresh_KD_LTD, threshold_factor
	RANGE ca_nmdai_max, cali_max, cati_max, cai_max, calcium, active_syn_flag, Cdur_init, Cdur_factor, w0, kernel, kernel_LTD
        RANGE last_dopamine, weight, KD1, KD2, KD1_min, KD2_min, n1, n2, KD_LTD, n_LTD, lthresh_LTP, hthresh_LTP, lthresh_LTD
...
PARAMETER {
	...
	learning_rate_w_LTP = 0.01
    	...
    	learning_rate_thresh_LTPl = 0.001
...
BREAKPOINT {
	...
	iNMDA = g*(v - Erev)
		...
		calcium = ca_nmdai_max
		calcium = cai_max/0.094
        } else {
	  ...
	  if (last_dopamine == 1 && active_syn_flag == 1) {
		 ...
		 :kernel = quadratic(calcium, KD1, KD2)
		 :kernel = lthresh(calcium, KD1, n1)* (1 - sigmoidal(calcium, hthresh_LTP, n2))
		 :kernel = custom_kernel(calcium, KD1, hthresh_LTP, n2)
		 :kernel = calcium/0.1 * (1 - sigmoidal(calcium, hthresh_LTP, n2))*1.0
		  kernel = lthresh(calcium, lthresh_LTP, n1)*hthresh(calcium, hthresh_LTP, n2)
		  
		  :normt = sigmoidal(0.5*(lthresh_LTP+hthresh_LTP), lthresh_LTP, n1*threshold_factor)* (1 - sigmoidal(0.5*(lthresh_LTP+hthresh_LTP), hthresh_LTP, n2*threshold_factor))
                 :kernelt = sigmoidal(calcium, lthresh_LTP, n1*threshold_factor)* (1 - sigmoidal(calcium, hthresh_LTP, n2*threshold_factor))*1.0/norm
		  
		  if (kernel < 0.0) { kernel = 0 }
		  :if (kernelt < 0.01) { kernelt = 0 }
		  
		  weight = weight + learning_rate_w_LTP * kernel 
		  lthresh_LTP = min(KD1_min + 0.015, lthresh_LTP + learning_rate_thresh_LTP * (cai_max+ 0.015 - lthresh_LTP))
		  :hthresh_LTP = lthresh_LTP + (KD2 - KD1)
		  hthresh_LTP = max(KD2_min - 0.01, hthresh_LTP + learning_rate_thresh_LTP * (cai_max-0.01 - hthresh_LTP))
		  ...
		  :weight = weight - learning_rate_w_LTD * cali_max/0.018285 * weight
		  ...
		  weight = weight - learning_rate_w_LTD * sigmoidal(cali_max, lthresh_LTD,n_LTD) * weight
		  :weight = weight - learning_rate_w_LTD * cali_max/0.018285 * sigmoidal(cali_max, lthresh_LTD,n_LTD) * weight
		  :weight = weight - learning_rate_w_LTD * kernel_LTD * weight
		  lthresh_LTP = min(KD1_min + 0.015, lthresh_LTP + learning_rate_thresh_LTPl * (cai_max+ 0.015 - lthresh_LTP))
          ...
          lthresh_LTP = 0.014285714285714297*weight + 0.015714285714285715
          hthresh_LTP = -0.2938775510204079*weight + 0.696734693877551 -0.15
          last_dopamine = dopamine		
          reset_max()
        }
...
FUNCTION custom_kernel(calcium, KD1, hthresh_LTP, n2) {
      if (calcium < KD1) {
          custom_kernel = calcium/KD1
      } else {
          custom_kernel = (1 - sigmoidal(calcium, hthresh_LTP, n2))
          }
}
