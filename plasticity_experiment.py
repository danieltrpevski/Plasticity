...
#
        for syn in self.cell.esyn:
            ...
            if p.plot_distributed_inputs:
                if (syn.type in ['adaptive_shom_NMDA','adaptive_shom_NMDA_stp',
                                 'adaptive_my_shom_NMDA',
                                 'adaptive_hom_NMDA'] and syn.clustered_flag == False):
    ...
    #                self.cati_agh[-1].record(syn.sec(syn.pos)._ref_cati, record_step)
            ...
            (syn.type in ['adaptive_cshom_NMDA', 'adaptive_hom_NMDA'] and syn.clustered_flag == False):
                if not p.long_simulation:
                    self.w_ampa.append(h.Vector())
                    self.w_ampa[-1].record(syn.obj._ref_weight, p.record_step_thresh)
                    syn.ref_var_ampa = self.w_ampa[-1]

                    if syn.type not in ['adaptive_hom_NMDA']:
                        self.lthresh_LTP.append(h.Vector())
...
#                    syn.ref_var_cati_max = self.cati_max[-1]
    ...
    def plot_results(self):
        ...
        if self.exptype in ['xor', 'xor_hom', 'xor_test_set', 'xor_gen', 'xor_shom_spillover',
                              ...
                              'xor_shom_my_spillover_stp', 'nfbp_inh', 'pattern']:
            #-----------------------------------------#
            ...
            axes_ba[0].plot(self.tv.to_python()[0:4*multiplier], self.vdlist[0].to_python()[p.first_training_input_start:4*multiplier+p.first_training_input_start], color = 'gray');
            axes_ba[0].plot(self.tv.to_python()[0:4*multiplier], self.vdlist[0].to_python()[-4*multiplier:], color = 'black');
            ...
            if type(self.dend_record_list) == list:
                ...
                for i in range(1,len(self.vdlist)):
                    if i//p.dends_per_plot == (i-1)//p.dends_per_plot:
                        ...
                        axes_ba[idx].plot(self.tv.to_python()[0:4*multiplier], self.vdlist[idx].to_python()[p.first_training_input_start:4*multiplier+p.first_training_input_start], color = 'gray')
                        axes_ba[idx].plot(self.tv.to_python()[0:4*multiplier], self.vdlist[idx].to_python()[-4*multiplier:], color = 'black')
            ...
            axes_ba[-1].plot(self.tv.to_python()[0:4*multiplier], self.vs.to_python()[p.first_training_input_start:4*multiplier+p.first_training_input_start], color = 'gray')
            axes_ba[-1].plot(self.tv.to_python()[0:4*multiplier], self.vs.to_python()[-4*multiplier:], color = 'black')
...
#
#                fig_cali_max = plt.figure();
#                ax_cali_max = fig_cali_max.add_subplot(1,1,1);
#                ax_cali_max.set_xlabel('t'); ax_cali_max.set_ylabel('cali_max');
#                legend = []
#                for i in range(0,len(self.cali_max)):
#                    ax_cali_max.plot(self.tout, self.cali_max[i])
#
#                ax_cali_max.legend(legend)
#                sns.despine()
#
#                fig_ca_nmdai_max = plt.figure();
#                ax_ca_nmdai_max = fig_ca_nmdai_max.add_subplot(1,1,1);
#                ax_ca_nmdai_max.set_xlabel('t'); ax_ca_nmdai_max.set_ylabel('ca_nmdai_max');
#                legend = []
#                for i in range(0,len(self.ca_nmdai_max)):
#                    ax_ca_nmdai_max.plot(self.tout, self.ca_nmdai_max[i])
#
#                ax_ca_nmdai_max.legend(legend)
#                sns.despine()
    ...
    def plot_distributed_inputs(self):
        ...
        if p.adaptive_distributed_inputs == True:
            if not p.correlated_distributed_inputs:
                ...
                plt.colorbar(scalar_map, label='Distance ($\mu$ m)')
...
#
                ...
                for i in range(0,p.distributed_input_size):
                    ...
                    color = colormap(h.distance(self.synlist_agh[i].pos, sec = self.synlist_agh[i].sec)/cmax)
                    ax_agh[-1].plot(self.tthresh, nmda, color = color)
                    if not p.long_simulation:
                        ax_cali_agh[-1].plot(self.tout, self.cali_agh[i], color = color)
                        ax_cai_agh[-1].plot(self.tout, self.cai_agh[i], color = color)
                        # ax_cai_nmda_agh[-1].plot(self.tout, self.cai_nmda_agh[i])
                        ax_lthresh_LTP_agh[-1].plot(self.tthresh, self.synlist_agh[i].ref_var_lthresh_LTP.to_python(), color = color)
                        ax_lthresh_LTD_agh[-1].plot(self.tthresh, self.synlist_agh[i].ref_var_lthresh_LTD.to_python(), color = color)
                        ax_v_agh[-1].plot(self.tv, self.v_agh[i], color = color)
...
#
    ...
    def plot_diffusion(self):
        ...
        for c1, c2, c3, c4 in zip(rcalbindin, rCaMC, rCaMN, rfixed):
        ...
        for c1, c2, c3, c4 in zip(rcacalbindin, rcaCaMC, rcaCaMN, rcafixed):
            ax_cacal.plot(c1); ax_cacamc.plot(c2); ax_cacamn.plot(c3); ax_cafixed.plot(c4)

    def plot_pf_inputs(self):
        figs_pf = []; ax_pf = []
        self.synlist_pf = self.get_synapse_list('adaptive_pf_NMDA', clustered_flag = True)
        for i,d in enumerate(self.dendstatobj.pf_inputs):
            if d != []:
                figs_pf.append(plt.figure())
                ax_pf.append(figs_pf[-1].add_subplot(111))
                ax_pf[-1].set_xlabel('t'); ax_pf[-1].set_ylabel('pf weight');
                ax_pf[-1].set_title('Dendrite %d' %(i+1))
                for j in range(i*p.pf_input_size, (i+1)*p.pf_input_size):
                    ax_pf[-1].plot(self.tout, self.synlist_pf[j].ref_var_nmda.to_python())

        return figs_pf, ax_pf
...
#                    sns.despine()
            ...
            if plot_what in ['wampa','wnmda','thresh_LTP', 'lthresh_LTP', 'hthresh_LTP',
                ...
                if plot_what in ['lthresh_LTP', 'hthresh_LTP']:
                    ...
                    if (i%10 == 0):
                        axess[-1].plot(tinds, peaks2, color = color, linestyle = '', marker = 'o', markersize = 2.0)
...
#        axes[-1].legend(legend)
        ...
        if plot_what in ['lthresh_LTP']:
            axess[-1].plot(tinds, p.hthresh_LTP*np.ones(len(tinds),), color = 'gray', linestyle = '--')
            axess[-2].plot(tinds, p.hthresh_LTP*np.ones(len(tinds),), color = 'gray', linestyle = '--')
...
#                print(self.xor_input_vectors[i][x].to_python())
        ...
        # dends_with_pf_inputs = 0
        # for pi in self.dendstatobj.pf_inputs:
        #     if pi == [1]:
        #         dends_with_pf_inputs +=1
        # for i in range(0,dends_with_pf_inputs):
        #     for x in range(0, p.pf_input_size):
        #         spike_times = []
        #         for t in self.merged_times[0:int(len(self.merged_times)/4.5)]:
        #             for sp in range(0, p.pf_num_spikes):
        #                 spike_times.append(rnd.uniform(t, t + p.pf_input_window))
        #         spike_times.sort()
        #         self.pf_input_vectors.append(h.Vector(spike_times))

        for i, v in enumerate(self.pf_input_vectors):
            self.pf_input.append(h.VecStim())
            self.pf_input[i].play(v)
...
#        time = p.first_training_input_start + p.training_input_length + p.time_to_reward
        ...
        return xor_input, pf_times, reward_times
...
#                            h.setpointer(h._ref_stimulus_flag, 'stimulus_flag', syn2.obj)
        ...
        #                        h.setpointer(h._ref_dopamine, 'dopamine', syn.obj)
            ...
            if self.exptype in ['xor', 'xor_hom', 'xor_gen', 'xor_spillover', 'xor_hom_spillover',
                                'xor_shom_spillover', 'xor_cshom_spillover', 'xor_sspillover',
                                'xor_spillover_test', 'xor_zahra_spillover','xor_shom_my_spillover',
                                'xor_shom_my_spillover_stp']:
...
#                            distributed_input_dends.append([dendlist[idx_dend], positions[idx_pos]])
                        ...
                        for i,d in enumerate(distributed_input_dends):
                            ...
                            elif self.exptype in ['xor_shom_spillover', 'xor_hom_spillover','xor_shom_my_spillover']:
    ...
    def create_fixed_weights(self):
        weights = [0.00051, 0.00006]*p.xor_input_size
        weights = weights + [0.00006, 0.00051]*p.xor_input_size
        weights = weights + [0.00006, 0.00051]*p.xor_input_size
        weights = weights + [0.00051, 0.00006]*p.xor_input_size
    ...
    def save_data_for_fig3(self):
        tv = self.tv.to_python()
        tc = self.t.to_python()
        vd_list = []
        for v in self.vdlist:
            vd_list.append(v.to_python())
        vs = self.vs.to_python()

        if self.exptype == 'xor_spillover':
            syntype = 'adaptive_NMDA'
        elif self.exptype == 'xor_hom_spillover':
            syntype = 'adaptive_hom_NMDA'
        synlist_n = self.get_synapse_list(syntype, clustered_flag = True)
        synlist_nc = [s for s in synlist_n if 'spine' in s.sec.name()]
        l = synlist_nc
        r = [si for s in l for si in re.findall("\[\d+\]", s.sec.name()) ]
        r = [ int(num) for elem in r for num in re.findall("\d+", elem)]
        ca_nmda = []
        ...
        for i in range(0,len(l)):
            ...
            if i==0 or (r[i] != r[i-1]):
                ca_nmda.append([])
                ca_nmda[-1].append(self.cai_nmda_in_syns[i].to_python())
                cal.append([])
                cal[-1].append(self.cali_in_syns[i].to_python())
            else:
                ca_nmda[-1].append(self.cai_nmda_in_syns[i].to_python())
                cal[-1].append(self.cali_in_syns[i].to_python())

        res_dict = {'dend_record_list': self.dend_record_list,
                    'vd_list': vd_list,
                    'vs': vs,
                    'ca_nmda': ca_nmda,
                    'cal': cal,
        ...
        to_save = json.dumps(res_dict)
        with open('data_fig3_red_banana.dat', 'w', encoding = 'utf-8') as f:
             json.dump(to_save, f)
...
#    def seti_xor_input_times(self, xor_input_times):
#        group = 0
#        for times_list in xor_input_times:
#            for t in times_list:
#                self.burst_input_nc[group].event(t+0.1)
#
#            group = group + 1
#
#        print("Start times for each xor input in the %d groups" % len(self.xor_input)
#        for xg in self.xor_input:
#            print [x.start for x in xg]
#        print("Start times for each xor input group"
#        print [x.start for x in self.burst_input]
##        group = 0
##        for times_list in xor_input_times:
##            for t in times_list:
##                h.cvode.event(t,(self.seti_xor_input_freq, (group, 0) ))
##            group = group + 1
#
#    def set_inburst_start_time(self, t, group):
#        for s in range(0, len(self.xor_input[group])):
##                self.xor_input[group][s].start = first_time
#            self.xor_input[group][s].start = t + rnd.uniform(0,30)
#
#        for d in range(0, len(self.distributed_input[group])):
##                self.xor_input[group][s].start = first_time
#            self.distributed_input[group][d].start = t + 30 + rnd.uniform(0,30)

#        for i in range(0,num_groups):
#            self.burst_input.append(h.NetStim())
#            self.burst_input[-1].number = 1
#            self.burst_input[-1].noise = 0.
#            self.burst_input_nc.append(h.NetCon(None, self.burst_input[-1]))
#            self.burst_input_nc[-1].weight[0] = 1.0
#
#            for s in range(0,input_size):
#                self.xor_input[i].append(h.NetStim())
#                self.xor_input[i][s].number = 1
#                self.xor_input[i][s].noise = 0.
##                self.xor_input[i][s].start = 1e10
#
##                self.xor_input[i][s].interval = p.high_interval
##                self.xor_input[i][s].number = p.high_rate*p.training_input_length*0.001
##                self.xor_input[i][s].noise = 1.
#
#                self.xor_nc[i].append(h.NetCon(self.burst_input[-1] , self.xor_input[i][s]))
#                self.xor_nc[i][s].weight[0] = 1.0
#
#            for d in range(0, p.distributed_input_size):
#                self.distributed_input[i].append(h.NetStim())
#                self.distributed_input[i][d].number = 1
#                self.distributed_input[i][d].noise = 0.
##                self.distributed_input[i][d].start = 1e10
#
#                self.distributed_input_nc[i].append(h.NetCon(self.burst_input[-1] ,
#                                                     self.distributed_input[i][d]))
#                self.distributed_input_nc[i][d].weight[0] = 1.0
#
#        # Set proper start times for bursts
#
#        group = 0
#        for times_list in self.xor_input_times:
#            if len(times_list):
...
#            else:
#                first_time = 1e10
#            self.burst_input[group].start = first_time
#
#            for s in range(0, len(self.xor_input[group])):
##                self.xor_input[group][s].start = first_time
#                self.xor_input[group][s].start = first_time + rnd.uniform(0,30)
#
#            for d in range(0, len(self.distributed_input[group])):
##                self.xor_input[group][s].start = first_time
#                self.distributed_input[group][d].start = first_time + 30 + rnd.uniform(0,30)
#            group = group + 1
