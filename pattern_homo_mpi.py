...
for t in tasks:
    ...
    ex = pe.Inhibitory_Experiment('pattern_homo', cell, dendstatobj = dendstatobj)
    ...
    synlist = ex.get_synapse_list('adaptive2_homo_inhexp2syn', clustered_flag = True, drive_type = 'i')
...
if rank == 0:
    ...
    for d1, d2, d3, inpd, w, t in zip(rd1, rd2, rd3, input_dends_list, weights, theta_inh):
        res1.extend(d1); res2.extend(d2); res3.extend(d3); res4.extend(inpd); res5.extend(w); res6.extend(t);
    rd1 = res1; rd2 = res2; rd3 = res3; input_dends_list = res4; weights = res5; theta_inh = res6
...
#                'tw': tw,
    ...
    with open('./results/pattern_homo.dat', 'w', encoding = 'utf-8') as f:
        json.dump(to_save, f)
